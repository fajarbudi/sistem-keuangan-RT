                ...
                <div class="d-flex flex-wrap">
                    <form class="mx-2" action="{{$baseURL}}" id="formSearch">
                        <div style="position: relative;">
                            <input class="form-control" id="fnotulensi_topik" type="text" placeholder="Cari Topik..." required="" name="notulensi_topik" value="{{(isset($vFilter['notulensi_topik'])) ? $vFilter['notulensi_topik'] : ''}}">
                            <a href="{{$baseURL}}" class="{{(!isset($vFilter['notulensi_topik'])) ? 'd-none' : ''}}">                          
                            ...
                            <tr>
                                ...
                                <td align="center">
                                    <div class="btn-group-vertical" role="group" aria-label="Basic example">
                                        <a class="btn btn-success" href="{{route('notulensi.detail', $val->notulensi_id)}}"><i class="fa fa-sign-in me-1"></i>Detail</a>
                                        @if (Auth::user()->can('admin', $user) || Auth::user()->can('sekertaris', $user))                                                       
...
<script>
    ...
    const update = (data) => {
        $('#formModal').modal('show');
        $('#formData').attr('action', `${baseUrl}/updateData/${data.notulensi_id}`);
