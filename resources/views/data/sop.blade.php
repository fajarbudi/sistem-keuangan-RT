                ...
                <div class="d-flex flex-wrap">
                    <form class="mx-2" action="{{$baseURL}}" id="formSearch">
                        <div style="position: relative;">
                            <input class="form-control" id="fsop_judul" type="text" placeholder="Cari Nama sop..." required="" name="sop_judul" value="{{(isset($vFilter['sop_judul'])) ? $vFilter['sop_judul'] : ''}}">
                            <a href="{{$baseURL}}" class="{{(!isset($vFilter['sop_judul'])) ? 'd-none' : ''}}">                          
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <strong>{{$val->sop_judul}}</strong>
                                    <br />
                                    {{$val->sop_isi}}
                                </td>
                                <td>
                                    <button onclick="show('{{url('storage/'.$val->sop_gambar)}}')" {{!$val->sop_gambar ? 'disabled' : ''}} class="btn btn-success">View</button>
                                </td>
                                <td>
                                    <button onclick="show('{{url('storage/'.$val->sop_pdf)}}')" {{!$val->sop_pdf ? 'disabled' : ''}} class="btn btn-success">View</button>
                                </td>
                                @can('admin', App\Models\User::class)
                                    <td>{{$val->sop_urutan}}</td>
                                    <td>{{$val->sop_publikasi}}</td>
                                @endcan
                                <td>
                                ...
                                <td class="d-flex d-row justify-content-center">
                                    <div class="btn-group-vertical" role="group" aria-label="Basic example">
                                        ...
                                        <button class="btn btn-sm btn-secondary" type="button" onclick='hapus({{$val -> sop_id}}, `{{$val -> sop_judul}}`)'><i class="fa fa-times"></i> Hapus</button>
                                    </div>
                                </td>
                                @endcan
                            </tr>
            ...
            <div class="modal-toggle-wrapper text-start dark-sign-up">
                <form id="formData" class=" needs-validation" novalidate="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            <div class="col-md-12">
                                <label class="form-label" for="sop_judul">Judul</label>
                                <input class="form-control" id="sop_judul" type="text" placeholder="Masukkan Judul..." required="" name="sop_judul">
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="sop_isi">Keterangan</label>
                                <textarea name="sop_isi" id="sop_isi" class="form-control" rows="5"></textarea>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="sop_gambar">Gambar</label>
                                <input class="form-control" id="sop_gambar" type="file" placeholder="Masukkan Gambar..." required="" name="sop_gambar">
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="sop_pdf">PDF</label>
                                <input class="form-control" id="sop_pdf" type="file" placeholder="Masukkan PDF..." required="" name="sop_pdf">
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="sop_urutan">Urutan</label>
                                <input class="form-control" id="sop_urutan" type="number" min="1" placeholder="Masukkan Urutan..." required="" name="sop_urutan">
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="sop_publikasi">Publikasi</label>
                                <select id="sop_publikasi" class="form-select form-select-sm" aria-label=".form-select-sm example" name="sop_publikasi">
                            ...
                            <div class="col-md-12">
                                <label class="form-label" for="sop_popup">Utama / Penting (Popup)</label>
                                <select id="sop_popup" class="form-select form-select-sm" aria-label=".form-select-sm example" name="sop_popup">
                                    <option value="">--Pilih--</option>
                                    <option value="Ya">Ya</option>
                                    <option value="Tidak">Tidak</option>
                                </select>
...
<script>
    ...
    const update = (data) => {
        $('#formModal').modal('show');
        $('#formData').attr('action', `${baseUrl}/updateData/${data.sop_id}`);
