                ...
                <div class="d-flex flex-wrap">
                    <form class="mx-2" action="{{$baseURL}}" id="formSearch">
                        <div style="position: relative;">
                            <input class="form-control" id="fjenis_iuran_nama" type="text" placeholder="Cari Nama Jenis Iuran..." required="" name="jenis_iuran_nama" value="{{(isset($vFilter['jenis_iuran_nama'])) ? $vFilter['jenis_iuran_nama'] : ''}}">
                            <a href="{{$baseURL}}" class="{{(!isset($vFilter['jenis_iuran_nama'])) ? 'd-none' : ''}}">                          
                            ...
                            <tr>
                                ...
                                <td align="center">
                                    <div class="btn-group-vertical" role="group" aria-label="Basic example">
                                        ...
                                        <button class="btn btn-sm btn-secondary" type="button" onclick='hapus({{$val -> jenis_iuran_id}}, `{{$val -> jenis_iuran_nama}}`)'><i class="fa fa-times"></i> Hapus</button>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
...
<script>
    ...
    const add = () => {
        ...
        $('#jenis_iuran_nama').val("");
    }

    const update = (data) => {
        $('#formModal').modal('show');
        $('#formData').attr('action', `${baseUrl}/updateData/${data.jenis_iuran_id}`);
