                    ...
                    <div class="d-flex flex-wrap">
                        <form class="mx-2" action="{{ $baseURL }}" id="formSearch">
                            <div style="position: relative;">
                                <input class="form-control" id="fjenis_uang_nama" type="text"
                                    placeholder="Cari Nama jenis Uang..." required="" name="jenis_uang_nama"
                                    value="{{ isset($vFilter['jenis_uang_nama']) ? $vFilter['jenis_uang_nama'] : '' }}">
                                <a href="{{ $baseURL }}"
                                    class="{{ !isset($vFilter['jenis_uang_nama']) ? 'd-none' : '' }}">
                                    <span style="position: absolute; top: 20%; right: 10px;">X</span>
                                </a>
                            </div>
                        </form>
                        <div class="mx-2">
                            <button class="btn btn-primary" type="button" onclick="$('#formSearch').submit()"><i
                                    class="fa fa-search"></i> <span class="sembunyikan">Search</span></button>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>Nama</th>
                                    {{-- <th>Tanggal</th> --}}
                                    <th class="text-center"><button class="btn btn-sm btn-danger" type="button"
                                            onclick="add()"><i class="icon-pencil-alt"></i> Tambah</button></th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <td align="center">
                                            <div class="btn-group-vertical" role="group" aria-label="Basic example">
                                                <button class="btn btn-sm btn-primary" type="button"
                                                    onclick="update({{ $val }})"
                                                    @if ($val->jenis_uang_nama == 'Iuran') disabled @endif><i
                                                        class="fa fa-pencil-square-o"></i> Update</button>
                                                <button class="btn btn-sm btn-secondary" type="button"
                                                    onclick='hapus({{ $val->jenis_uang_id }}, `{{ $val->jenis_uang_nama }}`)'
                                                    @if ($val->jenis_uang_nama == 'Iuran') disabled @endif><i
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row g-3">
                                <div class="col-md-12">
                                    ...
                                    <input class="form-control" id="jenis_uang_nama" type="text"
                                        placeholder="Masukkan Nama jenis_uang..." required="" name="jenis_uang_nama">
                                </div>
                            </div>
                        </div>
